...
function App() {
	...
	const [extendedFamilyQuestActive, setExtendedFamilyQuestActive] =
		useState(false);
	// outputs
	...
	const extraInfo = keptFish.length > 0 && (
		<div>
			<p class='lvl'>
				Lvl/Xp: {convertXpToLevel(endingFishXp)}/
				{Math.round(endingFishXp)}
			</p>
			<p class='time-in-day'>Time in day: {timeInDay}</p>
		</div>
	);
