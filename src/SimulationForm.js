	...
	const onSimulate = () => {
		...
		while (timeToSimulate > 0) {
			...
			for (let fish of Fish) {
				...
				if (
					((usingTrainingRod && fish.difficulty < 50) ||
						!usingTrainingRod) &&
					fishingLevel >= fish.requiredLevel
				) {
					...
					if (fishingArea & fish.location && weather & fish.weather) {
						...
						if (
							fishingArea &
								(baseLocations.GINGER_ISLAND |
									baseLocations.NIGHT_MARKET) ||
							season & fish.seasons
						) {
							...
							for (let i = 0; i < fish.time.length; i = i + 2) {
								...
								if (
									dayEnd - timeToSimulate / 0.6 >= minTime &&
									dayEnd - timeToSimulate / 0.6 <= maxTime
								) {
			...
			// ...perfect catch?
			let perfectChance =
				(caughtFish.difficulty -
					(usingTrainingRod ? 5 : fishingLevel) * 2.5) /
				100;
			...
			// XP
			let xpGain =
				3 + 3 * caughtFish.quality + (1 / 3) * caughtFish.difficulty;
			if (perfectCatch) xpGain *= 2.4;
	...
	return (
		<div className='simulationForm'>
			...
			<div className='formItem'>
				...
				<select
					id='locationSelect'
					onChange={(e) => setFishingArea(e.target.value)}
				>
			...
			<div className='formItem'>
				...
				<select
					id='tackleSelect'
					onChange={(e) => setFishingTackle(e.target.value)}
				>
			...
			<div className='formItem'>
				...
				<select
					id='baitSelect'
					onChange={(e) => setFishingBait(e.target.value)}
				>
			...
			<div className='formItem'>
				...
				<input
					id='trainingRod'
					type='checkbox'
					onChange={onTrainingRodChanged}
					checked={usingTrainingRod}
				/>
			</div>
			<div className='formItem'>
				...
				<input
					id='catchLegendary'
					type='checkbox'
					onChange={onCatchLegendaryChanged}
					checked={catchLegendaryFish}
				/>
			</div>
			<div className='formItem'>
